...
pub trait Dataflow {
    ...
    fn solve(&self, func: &Function) -> (Vec<BitVector>, Vec<BitVector>) {
        ...
        let mut worklist: Worklist<BlockId> = match Self::DIRECTION {
            Direction::Forward => (0..num_blocks).rev().map(BlockId).collect(),
            Direction::Backward => (0..num_blocks).map(BlockId).collect(),
        };
